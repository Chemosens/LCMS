#'@param nameXlsx name of the xlsx corresponding to the samples
#'@param blankXlsx name of the .xlsx corresponding to the blankx
#'@param timeNoise number by which multiplying the averaged noise: every intensity <timeNoise*averageNoise will be NA.
#'@importFrom openxlsx read.xlsx createWorkbook loadWorkbook addWorksheet writeData saveWorkbook sheets
removeBlank=function(nameXlsx="intokPOS.xlsx",blankXlsx="intBlank.xlsx",timeNoise=3,nameWb="correctedWithBlanks.xlsx")
{
  ...
  wb_blank=loadWorkbook(blankXlsx)
  ...
  for(class in classNames)
  {
    ...
    jdd_class_blank=read.xlsx(blankXlsx,sheet=class)
